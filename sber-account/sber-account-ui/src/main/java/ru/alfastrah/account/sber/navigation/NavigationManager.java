package ru.alfastrah.account.sber.navigation;
...
import ru.alfastrah.account.sber.view.board.ContractsView;
...
@Component
@UIScope
public class NavigationManager extends SpringNavigator implements HasLogger {
    ...
    public void navigateToDefaultView() {
        getLogger().trace("Enter to navigateToDefaultView {}", getState());
        /*if (!getState().isEmpty()) {
            return;
        }*/

        navigateTo(ContractsView.class);
    }

    public void navigateToAuthorizationView() {
        System.out.println("Enter to navigateToAuthorizationView");
        navigateTo(AuthorizationView.class);
    }
